...
public class PlayerChat : NetworkBehaviour
{
    ...
    [SyncVar(hook = nameof(UpdateUI))] public string otherMessage;
    ...
    {
        RaycastHit2D hit = Physics2D.Raycast(origin, direction, 100, bubbleMask);
        if (hit)
        {
            if(hit.collider.gameObject.TryGetComponent<ChatBubble>(out ChatBubble bubu))
            {
                bubu.WasHit();
                Debug.Log("I shot bubble");
            }
        }
    }
    ...
    {
        GameObject bub = Instantiate(chatBubble);
